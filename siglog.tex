...
\todo[inline,author=RH]{The biggest criticism overall from VV is that we do not represent the history fully and
  faithfully, that there are technical mistakes surrounding the use of the word ``proposition'' for other than a
  $-1$-type, esp. LEM, and that we do not state clearly what problems are solved and what remain open.  Because of his
  objections about the validity of HIT's, any statements involving them are, to him, conditional on their validity.  He
  also objects to subjective statements about beauty.  He feels that we've presented the subject as ``closed'' whereas
  there are lots of opportunities for more work.  And he feels that ``Univalent Framework'' is the right term to use for
  the foundational program he is pursuing that emphasizes machine-checked proof.  In his view the homotopy
  interpretation is secondary to the foundational program.  I think the breezy style, which I found to be engaging, rubs
  him up the wrong way.  (Please don't shoot the messenger!)  }
...
Streicher~\cite{HS} and then made explicit by Awodey and his students Lumsdaine and Warren~\cite{AW,L}, who showed that
every type in Martin-L\"{o}f's system has the structure of an $\infty$-groupoid, and that the entire system can be
interpreted in any Quillen model category, an abstract framework for doing homotopy theory.\todo[author=RH]{See below.}  The connection was clinched
by Voevodsky's introduction of the \emph{univalence axiom}, which is motivated by the homotopical interpretation, which
relates type equality to homotopy equivalence \cite{KLV}.
...
homotopical perspectives be called the \emph{Univalent Framework} unifying two major lines of development in
mathematics. A critically important point is that the univalent unification is \emph{fully compatible} with classical
mathematics.  Although it does not involve any principles that are incompatible with classical reasoning, it does, at
the outset, avoid postulating axioms that are unnecessary for much of the development.\todo[author=RH]{Clarify.}
Indeed, the formal system of homotopy type theory contains within it the whole of ZF set theory, with or without the
Axiom of Choice, and with or without the Law of the Excluded Middle, two principles that lie at the heart of classical
mathematics and that have, regrettably, been obstacles to the kind of unification described here.  By not insisting on
these axioms globally, it is possible to consider a far richer notion of type than has hitherto been possible in the
computational setting, namely one in which types are abstract spaces that can have non-trivial higher-dimensional
structure, as exemplified by familiar examples such as the $n$-spheres for all $n\geq 0$.  Rather than being encoded as
certain structured sets representing certain conceptions of space, such as a topological space, these objects instead
arise \emph{synthetically} within the theory.  They are characterized by their introductory and eliminatory forms
(mapping-in and mapping-out properties, or universal properties), rather than by how they are constructed as
sets.\todo[author=RH]{Perhaps clarify and relate to universal conditions?}  This perspective greatly simplifies many
familiar constructions in homotopy theory, such as homotopy groups~\cite{HoTTbook,LS-Circ}, truncations~\cite{HoTTbook},
and Eilenberg-MacLane spaces~\cite{LF-EM}, and supports remarkably simple and clear proofs\todo[author=RH]{Subjective}
of well-known results, such as calculations of the higher homotopy groups of the
spheres~\cite{LB-PinSn}.\todo[author=RH]{Depends on HIT's.  Maybe avoid subjective judgments, just say ``new proofs'' or
  similar.}  In this way it is possible to unify standard, classical logic and set theory with constructive logic and
type theory by regarding the former as the logic of a very special kind of objects (namely, sets), and the latter as the
general logic that applies not only to sets (the $0$-level spaces), but also to higher types of
spaces.\todo[author=RH]{Clarify propositions as $-1$-type, sets as $0$-types, relate to foundations based on predicate
  calculus and axioms.}
...
type, taken as a whole, corresponds to the \emph{total path space} of a space (see \cite{AW});\todo[author=RH]{only
  citation?}  since everything in the formal system respects identity, everything in the interpretation respects
homotopy, which is determined by identfication along paths.  Second, a characteristic feature of both intuitionistic
type theory and higher homotopy theory is the emphasis on \emph{structure} over \emph{property}.  In intuitionistic
...
structure of a type.\todo[author=RH]{Make all this more precise?}  Roughly speaking, the lowest level consists of the
types that have at most one element, called \emph{propositions}.  The next level, called \emph{sets}, consists of those
whose identity types are propositions. After that come the types whose identity types are sets: the \emph{groupoids}.
...
of \emph{equivalence of types} $A\simeq B$ to the respective cases of propositions, sets, and groupoids.  In this
setting, the new univalence axiom can be stated as the assertion that the type $A\simeq B$ of all equivalences between
...
theory, an infinite hierarchy of different ``homotopical dimensions" that were not fully developed until recently,
despite constructions such as Hofmann and Streicher's two-dimensional groupoid model~\cite{HS} that strongly hinted at
the importance of higher-dimensions.\todo[author=RH]{Note rephrasing.}  Type theory was of course originally conceived
...
ought that be the case?  Univalent foundations may provide a clue in the importance of proof-relevance, and the
