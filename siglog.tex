...
notion comes from Grothendieck's late conception of the homotopy types of spaces as represented by $\infty$-groupoids
...
that is motivated by the homotopical interpretation, and that relates type equality to homotopy equivalence~\cite{KLV,APW}.
...
%mathematics. 
Since Brouwer constructive founations have often been misconstrued as incompatible with classical mathematics.  By
...
been suggested by the computational approach, namely one in which types are abstract spaces that may have non-trivial
higher-dimensional structure, a leading example being the $n$-spheres for all $n\geq 0$.  Conventional foundations, such
as axiomatic set theory, encode these objects as elements of structured sets representing certain conceptions of space,
such as topological spaces as usually defined.  Here, instead, such higher-dimensional objects arise ``synthetically''
in much the way that Euclid's geometry embodies a notion of space in which lines and triangles are abstract mathematical
objects that are characterized by their behavior, rather than their representation.  This perspective clarifies some
familiar constructions in homotopy theory, such as characterizing the homotopy groups of some
spaces~\cite{HoTTbook,LS-Circ,LB-PinSn}, and constructing so-called Eilenberg-MacLane spaces~\cite{LF-EM} with specified
homotopy groups.  Moreover, these new proofs have a distinctively ``logical'', rather than ``geometric'' or
``topological'' flavor, arising from the way in which higher-dimensional spaces are represented using a so-called
...
homotopy invariant, in both the sense that type families and mappings between types inherently respect identifications
(paths, homotopies, or deformations) in a space, but they also respect an intrinsic notion of equivalence of types
corresponding to the homotopy equivalence of spaces.  This invariance may be observed externally to the language, but it
is also internalized by Martin-L\"{o}f's ingenious concept of the \emph{identity type}, which, as it turns out,
corresponds to the \emph{path space} of a space, and might therefore be called the \emph{identification type} to
emphasize its rich internal structure.  Second, a characteristic feature of both intuitionistic type theory and homotopy
theory is an emphasis on \emph{structure} over \emph{property}.  Under the propositions-as-types conception of
intuitionistic logic,\footnote{See, e.g., \cite{PAT} for a recent survey.} types express propositions, and objects of
the type are proofs of those propositions in the form of mathematical constructions that provide evidence for their
truth.  A similar emphasis can be discerned in abstract homotopy theory, in which, e.g., paths (homotopies) may be seen
...
are mathematical objects classified by a type (i.e., points in a space), and the homotopically motivated emphasis on
structure over property.  A further advantage accrues from the concept of proof relevance, namely that it naturally
supports a comprehensive approach to mechanized mathematics in which computer systems, such as Coq~\cite{coq}
or~\cite{Agda}, may be used to verify the correctness of mathematical arguments, of either a classical, set-theoretic
...
itself.  Once a proof has been obtained, others may not only verify its correctness by the usual means, but one may also
submit the proof to one's own checker to ensure that it is valid according to the rules of type theory.\footnote{Of
  course, one may question the meaningfulness of type theory itself.  But as G\"{o}del demonstrated such assumptions are
  unavoidable.}  This approach is fundamentally the same as that proposed by de Bruijn in the AUTOMATH
...
on a small trusted code base to ensure that validity of proofs.

\smallskip

Returning to the concept of identifications in a space, these lead to Voevodsky's conception of a hierarchy of
\emph{homotopy levels}, or \emph{h-levels} for short, definable within type theory.  Whereas the usual hierarchy of size
is determined by type universes or large cardinals in type theory or set theory, the hierarchy of h-levels is based
instead on the internal structure of a type.
%\todo[author=RH]{Make all this more precise?}  
Very roughly speaking, the lowest level consists of the types that have at most one element, up to path-connectedness;
these are called \emph{propositions}, and they correspond to the (empty or) contractible spaces.  The next level, called
\emph{sets}, consists of those types whose identity types are themselves propositions. After that come the types whose
identity types are sets: the \emph{groupoids}.  And so on.  Just the recognition that this hierarchy of h-levels is
present in the system of all types has been a huge advance in our understanding of type theory; previously, it was
simply a mystery that some types were fully determined by their elements, while others seemed to behave as though they
had some further structure.  The construction of quotient types, for example, is now greatly simplified when one knows
that the equivalence relation being factored out is a family of propositions, and not ``higher" types.  For another
example, for types $A$ and $B$ that are propositions, the relevant notion of equivalence is \emph{logical equivalence},
represented by the type $A\leftrightarrow B$.  For sets, the relevant notion is \emph{isomorphism} $A\cong B$, and for
groupoids, there is the notion of \emph{(categorical) equivalence} $A\simeq B$.  Each of these concepts results by
specializing the single, uniform notion of \emph{equivalence of types} $A\simeq B$ (also due to Voevodsky) to the
respective cases of propositions, sets, and groupoids.
